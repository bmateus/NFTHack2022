...
const { generateAlienName, titles } = require('./namegenerator');

const UPLOAD_DATA = false

//axios has an issue with uploading files > 5 MB
const IMAGE_BATCH_SIZE = 10;

// Moralis creds
const appId = "80zPLUSkIBfS7apoJENaueENE2lEkNIWrhnGrFeS";
const serverUrl = "https://y8emtia93i9r.bigmoralis.com:2053/server";
const masterKey = "iAK7Osx2yiY3hs4PybZezDvAEjbuu19ewCMs8JMR"; // DO NOT DISPLAY IN PUBLIC DIR
const xAPIKey = "k1Uf56ZrcVJOrbI"; // DO NOT DISPLAY IN PUBLIC DIR
...
const generateMetadata = (_dna, _edition, _attributesList, _path) => {
  ...
  let name = generateAlienName()

  if (Math.random() < 0.1 && titleIndex < titles.length - 1)
  {
    let title = " the " + titles[titleIndex];
...
const getAttributeForElement = (_element) => {
  //console.log("getAttributeForElement:", _element)
...
// upload metadata
const uploadMetadata = async (_cids) => {
  ...
  for (let i = 0; i < editionSize; i++) {
    ...
    let cid = _cids[Math.floor(i / IMAGE_BATCH_SIZE)];
...
const startCreating = async () => {
  ...
  while (editionCount <= editionSize) {
    ...
    const saveFile = async () => {
      ...
      console.log(
        "Fantom Martians " + editionCount.toString()
      );
  ...
  const processImageBatch = async () =>
  {
    ...
    if (UPLOAD_DATA)
    {
      try{
        ...
        console.log("IMAGE FILE PATHS:", response.data);
        const image_CID = response.data[0].path.split("/")[4];
        console.log("IMAGE CID:", image_CID);
        image_CIDs.push(image_CID);
        ipfsArray = [];
  ...
  for (let i = 0; i < editionCount; i++) 
  {
    ...
    const fileData = await readFile(`./output/${id}.png`);
    console.log("uploading image to ipfs:", id);
    ...
    if ( ipfsArray.length == IMAGE_BATCH_SIZE)
    {
...
// pass folder CID to meta data
  await uploadMetadata(image_CIDs);
...
//QmVrNF37nZxckzCr4RGNd6CnCGV5McYacunsF6o4cqAgzG
//QmNaRL1fB1xrws1Jt8s3vspsx225BzM1MHFme8BePZPD9c